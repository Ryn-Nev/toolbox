...
set "PY_URL=https://raw.githubusercontent.com/Ryn-Nev/toolbox/main/UV-processor/installation/UV-processor.py"
...
powershell -Command "try { Invoke-WebRequest -Uri '%PY_URL%' -OutFile '%INSTALL_DIR%\UV-processor.exe' } catch { Write-Host 'Error downloading UV-processor.exe'; exit 1 }"
if errorlevel 1 (
